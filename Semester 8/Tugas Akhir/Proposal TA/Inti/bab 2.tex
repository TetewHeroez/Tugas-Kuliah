...
\[
  ...
  \begin{pmatrix}
    ...
    \vdots      & \vdots      & \vdots      & \ddots & \vdots      \\
    \varepsilon & \varepsilon & \varepsilon & \cdots & 0
  \end{pmatrix}.
\]
...
\begin{proposisi}[\cite{buchinskiy2024analysisfour}]
  ...
  \begin{align*}
    C^{(t)} & = c_0 I_n \oplus c_1 (P^{(t)}) \oplus c_2 (P^{(t)})^{2} \oplus \cdots \oplus c_{n-1} (P^{(t)})^{n-1}, \\
    C_{(t)} & = c_0 I_n \oplus c_1 (P_{(t)}) \oplus c_2 (P_{(t)})^{2} \oplus \cdots \oplus c_{n-1} (P_{(t)})^{n-1}.
  \end{align*}
  dengan
  \begin{align*}
    P_{(t)} & =
...
\begin{figure}[H]
  ...
  \caption{Total ada 9 \emph{derangement} dari himpunan $S_4$}
...
\begin{teorema}
  Misalkan $A \in L_S(n)$ adalah sebuah matriks Latin berorde $n$ dengan himpunan simbol
  $\{a_1, a_2, \ldots, a_n\}$.
