...
- Codex update: slide prosedur konstruksi sekarang memblokir lanjutan algoritma sampai matriks A valid sebagai Latin square. Jika A belum valid, panel kandidat dan langkah-langkah B* -> B disembunyikan lalu diganti peringatan agar pengguna membetulkan baris/kolom dulu. Slide reduksi juga diperbarui dengan pasangan contoh baru sehingga ada entri hasil yang tepat bernilai `n+1 = 5`, supaya batas bawah minimum terlihat lebih konkret.
- Codex update: contoh reduksi dikoreksi lagi karena pasangan matriks sebelumnya ternyata masih belum menghasilkan entri 5. Sekarang baris pertama `A` dan kolom pertama `B` memberi penjumlahan `1+4`, `2+3`, `4+1`, dan `3+2`, sehingga maksimum entri hasil awal benar-benar tepat `5`.
