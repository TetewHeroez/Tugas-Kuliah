...
def centralizer(s):
    ...
    grup_kiri = MathTex(r"C_{S_9}(\sigma)", color=BLUE_E, font_size=40).move_to(LEFT * 3.5 + UP * 1.0)
    grup_kanan = MathTex(r"\mathbb{Z}_3^3 \rtimes S_3", color=RED_E, font_size=40).move_to(RIGHT * 3.5 + UP * 1.0)
    ...
    # Panah melengkung
    panah_atas = CurvedArrow(grup_kiri.get_top() + RIGHT*0.2 + UP*0.2, grup_kanan.get_top() + LEFT*0.2 + UP*0.2, angle=-TAU/8, color=BLACK)
    panah_bawah = CurvedArrow(grup_kanan.get_bottom() + LEFT*0.2 + DOWN*0.2, grup_kiri.get_bottom() + RIGHT*0.2 + DOWN*0.2, angle=-TAU/8, color=BLACK)
