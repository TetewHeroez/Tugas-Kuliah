...
class FlowchartToGantt(MovingCameraScene):
    def construct(self):
        # Konfigurasi Template Tex
        ...
        self.camera.frame.move_to(full_chart.get_center())
        self.camera.frame.set(width=full_width)
        
        # Animasi Pembuka (Dipercepat: 1s -> 0.5s)
        self.play(DrawBorderThenFill(nodes), Create(arrows), run_time=0.5)
        self.wait(0.25)
        ...
        # --- ANIMASI TRAVERSAL (ZOOM & PENJELASAN) ---
        ...
        for chunk in chunks:
            ...
            if len(current_desc_group) > 0:
                # Dipercepat: 0.3 -> 0.15
                self.play(FadeOut(current_desc_group), run_time=0.15)
                current_desc_group = VGroup()
            
            # Animasi Kamera & Dot Masuk (Dipercepat: 1 -> 0.5)
            self.play(
                self.camera.frame.animate.move_to(target_center).set(width=target_width),
                MoveAlongPath(travel_dot, entrance_arrow),
                run_time=0.5
            )
            self.remove(travel_dot)
            ...
            for i, idx in enumerate(chunk):
                ...
                if i > 0: 
                    ...
                    # Dipercepat: 1 -> 0.5
                    self.play(MoveAlongPath(internal_dot, internal_arrow), run_time=0.5)
                    self.remove(internal_dot)
            ...
            # self.next_slide() # Ganti wait jika pakai ManimSlides
            self.wait(0.5)
        ...
        # Animasi Restore & Dot Keluar (Dipercepat: 1 -> 0.5)
        self.play(
        ...
        # Transisi Elemen (Dipercepat: 1 -> 0.5)
        self.play(
            FadeOut(start_node),
            FadeOut(end_node),
            FadeOut(arrows),
        ...
        # Animasi Muncul Tabel (Dipercepat: 0.5 -> 0.25)
        self.play(
            self.camera.frame.animate.move_to(header_group.get_center() + DOWN*1.5).set(width=header_group.width * 1.3),
        ...
        # --- ISI TABEL (TRANSFORM FLOWCHART KE BAR) ---
        for i in range(len(target_rows)):
            row = target_rows[i]
            # Dipercepat: 0.3 -> 0.15
            self.play(Create(row[0]), Create(row[2]), Create(row[4]), run_time=0.15)
        ...
        # Animasi Akhir (Dipercepat: 1.5 -> 0.75)
        self.play(
