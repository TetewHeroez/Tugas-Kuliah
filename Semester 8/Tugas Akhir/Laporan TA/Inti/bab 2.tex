...
\begin{proof}
  ...
  \noindent Selanjutnya, akan ditunjukkan bahwa $\phi$ bersifat injektif dengan meninjau kernelnya. Misalkan $e$ adalah permutasi identitas pada $S_n$ dan $I_{\max}$ adalah elemen identitas pada $\mathcal{P}_n$. Kernel dari $\phi$ adalah $\ker(\phi) = \{\tau \in S_n \mid \phi(\tau) = I_{\max}\}$.
  Ambil sembarang $\tau \in \ker(\phi)$, maka $\phi(\tau) = P_{\tau^{-1}} = I_{\max}$.
  Karena $I_{\max}$ hanya dibangun oleh permutasi identitas, haruslah $\tau^{-1} = e$, yang berakibat $\tau = e$.
  ...
  Karena $\phi : S_n \to \mathcal{P}_n$ adalah fungsi injektif di antara dua himpunan berhingga dengan kardinalitas yang sama ($|S_n| = |\mathcal{P}_n|$), maka berdasarkan prinsip Dirichlet (Pigeonhole Principle), $\phi$ secara otomatis bersifat surjektif.
