...
Konsep tindakan grup pada suatu himpunan tak-kosong dapat diaplikasikan pada grup itu sendiri. Salah satu bentuk tindakan grup yang mendasar adalah tindakan grup pada dirinya sendiri melalui operasi konjugasi. Tindakan ini memunculkan relasi antar elemen yang membentuk suatu orbit spesifik, serta himpunan elemen yang menetap atau komutatif yang secara formal menstabilkan tindakan tersebut.
...
Secara kombinatorial, formula pada \autoref{lem:kardinalitas_centralizer_Sn} adalah akibat langsung dari syarat pelestarian struktur konjugasi $\tau a \tau^{-1} = a$. Komponen $k^{a_k}$ merepresentasikan banyaknya cara merotasi elemen secara internal di dalam $a_k$ sikel. Sementara itu, komponen $a_k!$ merepresentasikan banyaknya cara menukar silang letak antar sikel yang panjangnya identik. Pemetaan ke sikel dengan panjang berbeda dilarang mutlak karena dipastikan akan merusak tipe permutasi asalnya.
...
\begin{contoh}
  ...
  \]
  Kardinalitas ini berjumlah $8$, yang mana konsisten dengan formula analitik \autoref{lem:kardinalitas_centralizer_Sn} untuk tipe permutasi $[0, 2, 0, 0]$, yakni $|C_{S_4}(\sigma)| = 2^2(2!) = 8$.
\end{contoh}

\begin{contoh}
  Sebagai perluasan ilustrasi derajat kebebasan pemetaan $\tau$, tinjau permutasi $\sigma = (1\;2\;3)(4\;5\;6)(7\;8)(9) \in S_9$ dengan tipe permutasi $[1, 1, 2, 0, 0, 0, 0, 0, 0]$.
  Berdasarkan batasan pada \autoref{prop:konjugasi_sikel_Sn}, pembentukan anggota \emph{centralizer} diperoleh dari analisis per blok sikel:
  Sikel $(9)$ terkunci mutlak ($1$ cara rotasi). Sikel $(7\;8)$ terisolasi secara ukuran panjang ($2$ cara rotasi internal). Pasangan sikel identik $(1\;2\;3)$ dan $(4\;5\;6)$ memiliki kebebasan berotasi internal masing-masing $3$ kali sekaligus bertukar letak secara utuh ($3^2 \times 2! = 18$ cara).
  Kardinalitas \emph{centralizer} merupakan perkalian langsung dari derajat kebebasan pergerakan tersebut:
  \begin{align*}
    |C_{S_9}(\sigma)| & = \prod_{k=1}^{9} k^{a_k} (a_k!)                                = (1^1 \times 1!) \times (2^1 \times 1!) \times (3^2 \times 2!) = 1 \times 2 \times 18 = 36.
  \end{align*}
\end{contoh}
