...
\begin{enumerate}
                ...
                \begin{center}
                  \begin{tikzpicture}
                    \begin{axis}[
                      ...
                      % Titik-titik
                      % Variabel titik agar fleksibel dan bisa direlatifkan
                      \def\Qx{2} \def\Qy{2} \def\Qz{0}
                      \def\Rx{3} \def\Ry{-1} \def\Rz{5}
                      \def\Px{-1} \def\Py{2} \def\Pz{3}
                      \def\Sx{2} \def\Sy{-1} \def\Sz{8}

                      \coordinate (Q) at (axis cs:\Qx,\Qy,\Qz);
                      \coordinate (R) at (axis cs:\Rx,\Ry,\Rz);
                      \coordinate (P) at (axis cs:\Px,\Py,\Pz);
                      \coordinate (S) at (axis cs:\Sx,\Sy,\Sz);

                      % Hitung P_proj (proyeksi ortogonal P ke garis QR) secara matematis
                      \pgfmathsetmacro{\QRx}{\Rx - \Qx} \pgfmathsetmacro{\QRy}{\Ry - \Qy} \pgfmathsetmacro{\QRz}{\Rz - \Qz}
                      \pgfmathsetmacro{\QPx}{\Px - \Qx} \pgfmathsetmacro{\QPy}{\Py - \Qy} \pgfmathsetmacro{\QPz}{\Pz - \Qz}
                      \pgfmathsetmacro{\t}{(\QPx*\QRx + \QPy*\QRy + \QPz*\QRz) / (\QRx*\QRx + \QRy*\QRy + \QRz*\QRz)}
                      \pgfmathsetmacro{\Pjx}{\Qx + \t*\QRx} \pgfmathsetmacro{\Pjy}{\Qy + \t*\QRy} \pgfmathsetmacro{\Pjz}{\Qz + \t*\QRz}
                      \coordinate (P_proj) at (axis cs:\Pjx, \Pjy, \Pjz);

                      % Garis L
                      \addplot3[thick, blue, <->, forget plot] coordinates {(1.5, 3.5, -2.5) (3.5, -2.5, 7.5)} node[pos=0.9, above, text=blue] {$L$};

                      % Jajargenjang
                      \fill[gray!30, opacity=0.7] (Q) -- (R) -- (S) -- (P) -- cycle;
                      \draw[dashed, gray] (Q) -- (R) -- (S) -- (P) -- cycle;

                      % Vektor QR & QP
                      \draw[->, thick, red] (Q) -- (R) node[midway, below left] {$\vec{QR}$};
                      \draw[->, thick, red] (Q) -- (P) node[midway, right] {$\vec{QP}$};

                      % Vektor cross product (Normal)
                      % Arah: (-9, -8, -3), di-scale agar muat di grafik
                      \coordinate (N) at (axis cs:-0.7, 0, 2);
                      \node[below left, gray,rotate=-30] at (N) {$|\vec{QR} \times \vec{QP}|$};

                      % Jarak d
                      \draw[dashed, thick, black] (P) -- (P_proj) node[midway, above] {$d$};

                      % Tanda Siku-siku (sudut proyeksi ke L direlatifkan terhadap posisi baru P dan Q)
                      \pgfmathsetmacro{\lenQ}{sqrt((\Qx-\Pjx)^2 + (\Qy-\Pjy)^2 + (\Qz-\Pjz)^2)}
                      \pgfmathsetmacro{\lenP}{sqrt((\Px-\Pjx)^2 + (\Py-\Pjy)^2 + (\Pz-\Pjz)^2)}
                      \pgfmathsetmacro{\dQ}{0.25 / \lenQ} \pgfmathsetmacro{\dP}{0.25 / \lenP}
                      \pgfmathsetmacro{\Ax}{\Pjx + \dQ*(\Qx - \Pjx)} \pgfmathsetmacro{\Ay}{\Pjy + \dQ*(\Qy - \Pjy)} \pgfmathsetmacro{\Az}{\Pjz + \dQ*(\Qz - \Pjz)}
                      \pgfmathsetmacro{\Cx}{\Pjx + \dP*(\Px - \Pjx)} \pgfmathsetmacro{\Cy}{\Pjy + \dP*(\Py - \Pjy)} \pgfmathsetmacro{\Cz}{\Pjz + \dP*(\Pz - \Pjz)}
                      \pgfmathsetmacro{\Bx}{\Ax + \Cx - \Pjx} \pgfmathsetmacro{\By}{\Ay + \Cy - \Pjy} \pgfmathsetmacro{\Bz}{\Az + \Cz - \Pjz}
                      \draw (axis cs:\Ax, \Ay, \Az) -- (axis cs:\Bx, \By, \Bz) -- (axis cs:\Cx, \Cy, \Cz);

                      % Label Titik
                      \node[circle, fill=black, inner sep=1.5pt, label=below:$Q$] at (Q) {};
                      \node[circle, fill=black, inner sep=1.5pt, label=below:$R$] at (R) {};
                      \node[circle, fill=black, inner sep=1.5pt, label=above:$P$] at (P) {};

                      % Garis z nembus ke jajar genjang
                      \draw[] (axis cs:0, 0, 3) -- (axis cs:0, 0, 6);
                    \end{axis}
                  \end{tikzpicture}
                \end{center}
  ...
  \item \begin{enumerate}[label=\alph*.]
          \item Fungsi vektor lintasan partikel adalah $\vec{r}(t) = (2\sin t + 2)\hat{i} + (3\cos t)\hat{j} + t\hat{k}$.
                Untuk mengetahui bentuk kurvanya, kita dapat mencari hubungan antara variabel $x$ dan $y$.
                Diketahui $x = 2\sin t + 2 \implies \frac{x-2}{2} = \sin t$ dan $y = 3\cos t \implies \frac{y}{3} = \cos t$.
                Dengan menggunakan identitas trigonometri $\sin^2 t + \cos^2 t = 1$, kita dapat mensubstitusikannya menjadi:
                \[ \left(\frac{x-2}{2}\right)^2 + \left(\frac{y}{3}\right)^2 = 1 \]
                Persamaan di atas merupakan persamaan sebuah silinder eliptik (elips) yang sejajar memanjang di sepanjang sumbu z (pusat di koordinat $x=2, y=0$). Karena $z = t$, titik pada partikel akan terus naik seiring waktu sambil bergerak melingkari elips tersebut. Jadi lintasan ini berbentuk sebuah \textit{elliptical helix}.
                \begin{center}
                  \begin{tikzpicture}[x={(-0.6cm,-0.4cm)}, y={(1cm,0cm)}, z={(0cm,0.7cm)}]
                    % Sumbu x, y, z
                    \draw[->, thick] (0,0,0) -- (6,0,0) node[below] {$x$};
                    \draw[->, thick] (0,-4,0) -- (0,4,0) node[right] {$y$};
                    \draw[->, thick] (0,0,0) -- (0,0,7.5) node[above] {$z$};

                    % Label angka sumbu x
                    \foreach \x in {1,2,3,4} {
                        \draw (\x,-0.1,0) -- (\x,0.1,0);
                        \node[anchor=north east, font=\scriptsize] at (\x,0,0) {\x};
                      }
                    % Label angka sumbu y
                    \foreach \y in {-3,-2,-1,1,2,3} {
                        \draw (0,\y,-0.1) -- (0,\y,0.1);
                        \node[anchor=north, font=\scriptsize] at (0,\y,0) {\y};
                      }
                    % Label angka sumbu z
                    \foreach \z in {1,2,3,4,5,6} {
                        \draw (-0.1,0,\z) -- (0.1,0,\z);
                        \node[anchor=east, font=\scriptsize] at (0,0,\z) {\z};
                      }

                    % Proyeksi keliling elips di dasar (bidang xy)
                    \draw[dashed, gray, thick, domain=0:360, samples=150, variable=\t]
                    plot ({2*sin(\t) + 2}, {3*cos(\t)}, {0});

                    % Lintasan heliks / lintasan partikel
                    \draw[thick, red, domain=0:360, samples=150, variable=\t]
                    plot ({2*sin(\t) + 2}, {3*cos(\t)}, {\t*pi/180});

                    % Garis putus-putus penghubung t=2\pi ke t=0
                    \draw[dashed, orange] (2,3,6.28318) -- (2,3,0);

                    % Titik penanda t = 0, pi, 2pi
                    \node[circle, fill=black, inner sep=1pt, label={[font=\tiny]right:$t=0$}] at (2,3,0) {};
                    \node[circle, fill=black, inner sep=1pt, label={[font=\tiny]left:$t=\pi$}] at (2,-3,3.14159) {};
                    \node[circle, fill=black, inner sep=1pt, label={[font=\tiny]right:$t=2\pi$}] at (2,3,6.28318) {};
                  \end{tikzpicture}
                \end{center}
  ...
  \item \begin{enumerate}[label=\alph*.]
          \item Substitusi variabel akan membantu menelusuri lintasan parameter $x = t - \sin t$, $y = 1 - \cos t$, dan $z = 4\sin(t/2)$.
                Untuk mengetahui bentuk bidang lintasannya, kita dapat mencari hubungan fungsional antara $y$ dan $z$.
                Dari identitas trigonometri sudut penengahan, diketahui hubungan $1 - \cos t = 2\sin^2\left(\frac{t}{2}\right)$.
                Substitusikan identitas tersebut ke persamaan komponen ordinat $y$:
                \[ y = 2\sin^2\left(\frac{t}{2}\right) \]
                Dari komponen ordinat $z$, persamaan $z = 4\sin(t/2)$ dapat ditulis ulang sebagai $\sin(t/2) = \frac{z}{4}$.
                Selanjutnya, kita akan mensubstitusikan nilai $\sin(t/2)$ tersebut ke dalam persamaan $y$:
                \[ y = 2\left(\frac{z}{4}\right)^2 = 2\left(\frac{z^2}{16}\right) = \frac{z^2}{8} \implies z^2 = 8y \]
                Oleh karena itu, bisa disimpulkan bahwa kurva ruang ini sebenarnya secara keseluruhan terhimpit sepenuhnya pada silinder parabola ($z^2 = 8y$). Sementara komponen translasi mendatarnya ($x = t - \sin t$) merambat melengkung layaknya fungsi \textit{cycloid} normal mengikuti bentuk parabola ini.
                \begin{center}
                  \begin{tikzpicture}[x={(-0.6cm,-0.4cm)}, y={(1cm,0cm)}, z={(0cm,1cm)}]
                    % Sumbu x, y, z
                    \draw[->, thick] (0,0,0) -- (14,0,0) node[below] {$x$};
                    \draw[->, thick] (0,0,0) -- (0,3.5,0) node[right] {$y$};
                    \draw[->, thick] (0,0,-5.5) -- (0,0,5.5) node[above] {$z$};
                    ...
                    % Proyeksi parabola di bidang yz (x=0)
                    \draw[dashed, gray, thick, domain=0:720, samples=150, variable=\t]
                    plot ({0}, {1 - cos(\t)}, {4*sin(\t/2)});

                    % Lintasan aktual 3D cycloid
                    \draw[thick, blue, domain=0:720, samples=200, variable=\t]
                    plot ({\t*pi/180 - sin(\t)}, {1 - cos(\t)}, {4*sin(\t/2)});
                  \end{tikzpicture}
                \end{center}
